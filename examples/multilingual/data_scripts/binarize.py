...
if SPM_PATH is None or not SPM_PATH.strip():
    print(
        "Please install sentence piecence from https://github.com/google/sentencepiece and set SPM_PATH pointing to the installed spm_encode.py. Exitting..."
    )
    sys.exit(-1)
...
if not os.path.exists(SPM_MODEL):
    call(
        f"wget https://dl.fbaipublicfiles.com/fairseq/models/mbart50/sentence.bpe.model -O {SPM_MODEL}"
    )


if not os.path.exists(SPM_VOCAB):
    call(
        f"wget https://dl.fbaipublicfiles.com/fairseq/models/mbart50/dict_250k.txt -O {SPM_VOCAB}"
    )
...
def encode_spm(
    model, direction, prefix="", splits=["train", "test", "valid"], pairs_per_shard=None
):
    src, tgt = direction.split("-")

    for split in splits:
        src_raw, tgt_raw = (
            f"{RAW_DIR}/{split}{prefix}.{direction}.{src}",
            f"{RAW_DIR}/{split}{prefix}.{direction}.{tgt}",
        )
...
def binarize(
    ...
    if pairs_per_shard is None:
        binarize_(
            bpe_dir, bpe_databin_dir, direction, spm_vocab=spm_vocab, splits=splits
        )
        ...
        binarize_(
            ...
            direction,
            spm_vocab=spm_vocab,
            splits=[s for s in splits if s != "train"],
        )
        for shard_bpe_dir in glob.glob(f"{bpe_dir}/shard*"):
            ...
            for test_data in glob.glob(f"{bpe_databin_dir}/valid.*") + glob.glob(
                f"{bpe_databin_dir}/test.*"
            ):
