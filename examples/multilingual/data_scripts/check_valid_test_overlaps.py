...
def check_train_sentences(
    src_path, tgt_path, direction, all_test_data, mess_up_train={}
):
    # src, tgt = direction.split('-')
...
def check_train_all(raw_data, directions, all_test_data):
    ...
    for direction in directions:
        ...
        _, size, overlapped_size_counted_dup = check_train_sentences(
            src_path, tgt_path, direction, all_test_data, mess_up_train
        )
...
def main():
    ...
    parser.add_argument(
        "--test-data", type=str, required=True, help="the test data folder "
    )
    ...
    all_test_data, test_data = get_all_test_data(
        args.test_data, directions, split="test"
    )
