...
def train_val_test_split(tsv_lines, km_lines, valid_percent, test_percent, seed=42):
    ...
    utt_ids, valid_utt_ids, _, _ = train_test_split(
        utt_ids, utt_ids, test_size=valid_percent, shuffle=True, random_state=seed
    )
    train_utt_ids, test_utt_ids, _, _ = train_test_split(
        utt_ids, utt_ids, test_size=test_percent, shuffle=True, random_state=seed
    )

    train_idx = [
        i for i, line in enumerate(tsv_lines) if get_utt_id(line) in train_utt_ids
    ]
    valid_idx = [
        i for i, line in enumerate(tsv_lines) if get_utt_id(line) in valid_utt_ids
    ]
    test_idx = [
        i for i, line in enumerate(tsv_lines) if get_utt_id(line) in test_utt_ids
    ]

    train_tsv, train_km = [tsv_lines[i] for i in train_idx], [
        km_lines[i] for i in train_idx
    ]
    valid_tsv, valid_km = [tsv_lines[i] for i in valid_idx], [
        km_lines[i] for i in valid_idx
    ]
    test_tsv, test_km = [tsv_lines[i] for i in test_idx], [
        km_lines[i] for i in test_idx
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="percent to allocate to validation set",
    )
    parser.add_argument(
        "--test-percent",
        type=float,
        default=0.05,
        help="percent to allocate to test set",
    )
    ...
    train_tsv, train_km, valid_tsv, valid_km, test_tsv, test_km = train_val_test_split(
        tsv, km, args.valid_percent, args.test_percent, args.seed
    )

    assert len(train_tsv) + len(valid_tsv) + len(test_tsv) == len(tsv)
    assert (
        len(train_tsv) == len(train_km)
        and len(valid_tsv) == len(valid_km)
        and len(test_tsv) == len(test_km)
    )
