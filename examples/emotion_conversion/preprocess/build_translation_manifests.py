...
def main():
    ...
    parser.add_argument(
        "data",
        type=Path,
        help="path to a dir containing .tsv and .km files containing emov dataset",
    )
    parser.add_argument(
        "output_path",
        type=Path,
        help="output directory with the manifests will be created",
    )
    parser.add_argument(
        "-cs",
        "--cross-speaker",
        action="store_true",
        help="if set then translation will occur also between speakers, meaning the same sentence can be translated between different speakers (default: false)",
    )
    parser.add_argument(
        "-dd",
        "--dedup",
        action="store_true",
        help="remove repeated tokens (example: 'aaabc=>abc')",
    )
    parser.add_argument(
        "-sh", "--shuffle", action="store_true", help="shuffle the data"
    )
    parser.add_argument(
        "-ae",
        "--autoencode",
        action="store_true",
        help="include training pairs from the same emotion (this includes examples of the same sentence uttered by different people and examples where the src and trg are the exact same seq)",
    )
    parser.add_argument(
        "-dr", "--dry-run", action="store_true", help="don't write anything to disk"
    )
    parser.add_argument(
        "-zs",
        "--zero-shot",
        action="store_true",
        help="if true, the denoising task will train on the same splits as the translation task (split by utterance id). if false, the denoising task will train on randomly sampled splits (not split by utterance id)",
    )
    parser.add_argument("--km-ext", default="km", help="")
    parser.add_argument(
        "--dict",
        default="/checkpoint/felixkreuk/datasets/emov/manifests/emov_16khz/fairseq.dict.txt",
        help="",
    )
    ...
    translation_dir = Path(args.output_path) / (
        "emov_multilingual_translation" + suffix + translation_suffix
    )
    os.makedirs(translation_dir, exist_ok=True)
    denoising_dir = Path(args.output_path) / (
        "emov_multilingual_denoising" + suffix + denoising_suffix
    )
    os.makedirs(denoising_dir, exist_ok=True)

    denoising_data = [
        p.name for p in (args.data / "denoising").glob("*") if "emov" not in p.name
    ]
    ...
    for split in ["train", "valid", "test"]:
        ...
        for EMOTION in EMOTIONS:
            ...
            open(denoising_dir / f"files.{split}.{EMOTION}", "w").writelines(
                [root] + emotion_tsv
            )
        ...
        for data in denoising_data:
            with open(
                args.data / "denoising" / data / f"{split}.{args.km_ext}", "r"
            ) as f1:
                with open(denoising_dir / f"{split}.{data}", "w") as f2:
                    f2.writelines(
                        [l if not args.dedup else dedup(l) for l in f1.readlines()]
                    )
        ...
        for SRC_EMOTION in EMOTIONS:
            TRG_EMOTIONS = (
                EMOTIONS if args.autoencode else set(EMOTIONS) - set([SRC_EMOTION])
            )
            for TRG_EMOTION in TRG_EMOTIONS:
                ...
                for i, tsv_line in enumerate(tsv_lines):
                    ...
                    assert (
                        speaker in SPEAKERS
                    ), "unknown speaker! make sure the .tsv contains EMOV data"
                ...
                for speaker, utt_ids in spkr2utts.items():
                    for utt_id, indices in utt_ids.items():
                        ...
                        pairs = [
                            (x, y)
                            for (x, y) in pairs
                            if get_emotion(tsv_lines[x]) == SRC_EMOTION
                            and get_emotion(tsv_lines[y]) == TRG_EMOTION
                        ]

                        for idx1, idx2 in pairs:
                            assert get_utt_id(tsv_lines[idx1]) == get_utt_id(
                                tsv_lines[idx2]
                            )
                            ...
                            km_line_idx1 = (
                                km_line_idx1 if not args.dedup else dedup(km_line_idx1)
                            )
                            km_line_idx2 = (
                                km_line_idx2 if not args.dedup else dedup(km_line_idx2)
                            )
                ...
                # create files
                os.makedirs(
                    translation_dir / f"{SRC_EMOTION}-{TRG_EMOTION}", exist_ok=True
                )
                open(
                    translation_dir
                    / f"{SRC_EMOTION}-{TRG_EMOTION}"
                    / f"files.{split}.{SRC_EMOTION}",
                    "w",
                ).writelines([root] + src_tsv)
                open(
                    translation_dir
                    / f"{SRC_EMOTION}-{TRG_EMOTION}"
                    / f"files.{split}.{TRG_EMOTION}",
                    "w",
                ).writelines([root] + trg_tsv)
                open(
                    translation_dir
                    / f"{SRC_EMOTION}-{TRG_EMOTION}"
                    / f"{split}.{SRC_EMOTION}",
    ...
    for SRC_EMOTION in EMOTIONS:
        ...
        for TRG_EMOTION in TRG_EMOTIONS:
            translation_preprocess(
                translation_dir / f"{SRC_EMOTION}-{TRG_EMOTION}",
                SRC_EMOTION,
                TRG_EMOTION,
                args.dict,
            )  # , only_train=SRC_EMOTION==TRG_EMOTION)
