...
def synth(argv, interactive=False):
    ...
    parser.add_argument(
        "--result-path", type=Path, help="Translation Model Output", required=True
    )
    parser.add_argument(
        "--data",
        type=Path,
        help="a directory with the files: src.tsv, src.km, trg.tsv, trg.km, orig.tsv, orig.km",
    )
    parser.add_argument(
        "--orig-tsv",
        default="/checkpoint/felixkreuk/datasets/emov/manifests/emov_16khz/data.tsv",
    )
    parser.add_argument(
        "--orig-km",
        default="/checkpoint/felixkreuk/datasets/emov/manifests/emov_16khz/core_manifests/emov_16khz_km_100/data.km",
    )

    parser.add_argument(
        "--checkpoint-file", type=Path, help="Generator Checkpoint", required=True
    )
    parser.add_argument(
        "--dur-model",
        type=Path,
        help="a token duration prediction model (if tokens were deduped)",
    )
    ...
    _, src_code_to_fname = get_code_to_fname(
        f"{a.data}/files.{a.split}.{a.src_emotion}",
        f"{a.data}/{a.split}.{a.src_emotion}",
    )
    _, tgt_code_to_fname = get_code_to_fname(
        f"{a.data}/files.{a.split}.{a.trg_emotion}",
        f"{a.data}/{a.split}.{a.trg_emotion}",
    )
    ...
    fname_to_idx = {
        orig_tsv_root + "/" + line.split("\t")[0]: i for i, line in enumerate(orig_tsv)
    }
    ...
    for i, (sid, result) in tqdm(enumerate(results)):
        ...
        src_spkr = (
            src_spkr
            if src_spkr in EMOV_SPK2ID
            else random.choice(list(EMOV_SPK2ID.keys()))
        )
        ...
        src_tokens_dur_pred = dur_models[trg_style_str].inflate_input(
            src_tokens_dur_pred
        )
        ...
        pred_src_f0 = f0_model.inference(
            torch.LongTensor(src_tokens).to(a.device), src_spkr, trg_style
        ).unsqueeze(0)
        pred_src_dur_pred_f0 = f0_model.inference(
            torch.LongTensor(src_tokens_dur_pred).to(a.device), src_spkr, trg_style
        ).unsqueeze(0)
        pred_gen_f0 = f0_model.inference(
            torch.LongTensor(gen_tokens).to(a.device), src_spkr, trg_style
        ).unsqueeze(0)
        pred_trg_f0 = f0_model.inference(
            torch.LongTensor(trg_tokens).to(a.device), src_spkr, trg_style
        ).unsqueeze(0)
        ...
        shutil.copy(
            src_code_to_fname[code_to_str(result["S"])],
            outdir / "0-source" / f"{sid}.wav",
        )

        audio = generate_from_code(
            generator,
            h,
            src_tokens,
            spkr=src_spkr,
        ...
        sf.write(
            outdir / "1-src-tokens-src-style-src-f0" / f"{sid}.wav",
            audio,
        ...
        sf.write(
            outdir / "2-src-tokens-trg-style-src-f0" / f"{sid}.wav",
            audio,
            samplerate=h.sampling_rate,
        )
        ...
        sf.write(
            outdir / "2.5-src-tokens-trg-style-src-f0" / f"{sid}.wav",
            audio,
            samplerate=h.sampling_rate,
        )
        ...
        audio = generate_from_code(
            ...
            src_tokens_dur_pred,
            spkr=src_spkr,
            f0=pred_src_dur_pred_f0,
            gst=trg_style,
            device=a.device,
        )
        sf.write(
            outdir / "3-src-tokens-trg-style-pred-f0" / f"{sid}.wav",
            audio,
            samplerate=h.sampling_rate,
        )
        ...
        sf.write(
            outdir / "4-gen-tokens-trg-style-pred-f0" / f"{sid}.wav",
            audio,
            samplerate=h.sampling_rate,
        )

        shutil.copy(
            tgt_code_to_fname[code_to_str(result["T"])],
            outdir / "5-target" / f"{sid}.wav",
        )
