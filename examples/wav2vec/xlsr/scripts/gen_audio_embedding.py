...
if __name__ == "__main__":
    ...
    state["cfg"]["model"]["w2v_path"] = (
        state["cfg"]["model"]["w2v_path"]
        .replace(
            "/checkpoint/arbabu/XLSR2/model_versions/", "/fsx/data/model_versions/"
        )
        .replace(
            "/checkpoint/kushall/final_model_checkpoints/wav2vec2/",
            "/fsx/data/wav2vec_ckpt/",
        )
    )
    state["cfg"]["task"]["data"] = state["cfg"]["task"]["data"].replace(
        "/checkpoint/kushall/data/", "/fsx/data/"
    )
    ...
    with torch.no_grad():
        for _, sample in tqdm(enumerate(itr)):
            ...
            list_src_len.extend(
                (~sample["net_input"]["padding_mask"]).sum(1).cpu().numpy()
            )
            latents = []
            logits = []
    ...
    print(
        f"| {list_uttname.shape=}, {list_latent.shape=}, {list_target.shape=}, {list_logit.shape=}, {list_src_len.shape=}"
    )
